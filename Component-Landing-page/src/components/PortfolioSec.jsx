    ...
    return (
        <section className="p-5" id="portfolio">
            <div className="container">
                ...
                <ul className="d-flex list gap-4 my-5 list-unstyled justify-content-center">
                    {filters.map(({ label, key }) => (
                        <li key={key} onClick={() => setFilterKey(key)}>
                            <p className={`text-body-secondary fw-semibold pointer ${filterKey === key ? "active" : ""}`}>
                                {label}
                            </p>
                        </li>
                    ))}
                ...
                <div className="row grid gy-5 justify-content-between">
                    {portfolioData.map((item, index) => (
                        <div
                            key={index}
                            className={`col-xl-4 col-md-6 portfolio-item grid-item ${item.category} ${item.comingSoon ? "" : "rounded shadow"}`}
                        >
                            {item.comingSoon ? (
                                <h4>Coming Soon</h4>
                            ) : (
                                <>
                                    ...
                                    <div className="portfolio-info p-2 rounded w-75">
                                        <h4>{item.title}</h4>
